...
exports.getDailyLogins = async (req, res) => {
  try {
    ...
    const users = await User.find(
      {},
      { username: 1, loginTimes: 1, logoutTimes: 1 }
    ); // Fetch only username, login times, and logout times
...
exports.getDailyLogouts = async (req, res) => {
  try {
    ...
    const users = await User.find(
      ...
      { username: 1, "logoutTimes.timestamp": 1 }
    ); // Fetch only username and logout times
          ...
          fs.unlink(filePath, (unlinkErr) => {
            if (unlinkErr) {
              ...
              return res.status(500).send({
                message:
                  "File deleted from DB, but error removing it from filesystem",
                error: unlinkErr,
              });
            }
...
exports.updateCampaignStatus = async (req, res) => {
  try {
    ...
    const updatedCampaign = await CampaignSchema.findByIdAndUpdate(
      id,
      { campaignStatus: status }, // Update the status field
